...
std::vector<int> load_wav(const std::string &wav_file_path, int *sample_rate, WavHeader *return_header) {
  // open file
...
WavHeader getHeader(int i, std::vector<int> data, const std::vector<Segment> segments, WavHeader header_entire) {
  WavHeader header;
  header.chunkId[4] = {'R', 'I', 'F', 'F'};
  header.chunkSize = (segments[i].stop - segments[i].start) * header_entire.bitsPerSample - 8;
  header.format = {'W', 'A', 'V', 'E'};
  header.subchunk1Id = {'f', 'm', 't'};
  ...
  header.subchunk2Id[4] = {'d', 'a', 't', 'a'};
  header.subchunk2Size = (segments[i].stop - segments[i].start) * header_entire.bitsPerSample - 44;
}

std::vector<int> getData(int i, const std::vector<int> data, const std::vector<Segment> segments) {
  std::vector<int> audio_segment;
  for(int i = segments[i].start, j = 0; i < segments[i].stop - 1; i++, j++) {
    audio_segment[j] = data[i];
  }
  return audio_segment;
}

int save_wav(int i, const std::vector<int> data, const std::vector<Segment> segments, WavHeader header) {
  ofstream myfile;
  myfile.open((to_string(i) + ".wav"), ios::binary);
  WavHeader segment_header = getHeader(i, data, segments, header);
  std::vector<int> segment_data = getData(i, data, segments);
  cout << "LOL" << endl;
  myfile << segment_header.chunkId << segment_header.chunkSize << segment_header.format << segment_header.subchunk1Id << segment_header.subchunk1Size << 
  segment_header.audioFormat << segment_header.numChannels << segment_header.sampleRate << segment_header.byteRate << segment_header.blockAlign << segment_header.bitsPerSample <<
  segment_header.subchunk2Id << segment_header.subchunk2Size;
  for (int i = 0; i < segment_data.size() - 1; i++) {
    myfile << segment_data[i];
  }
  myfile.close();
  return 0;
}
...
int main(int argc, char** argv) {
  ...
  int sample_rate = 0;
  WavHeader header;
  std::vector<int> audio = load_wav(wav_file_path, &sample_rate, &header);
  ...
  for(int i = 0; i < segments.size() - 1; i++) {
    save_wav(i, audio, segments, header);
  }
